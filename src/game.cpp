...
Game_c::Game_c(){
    ...
    /* piece surfaces */
    bPSurface = IMG_Load("res/BP.png"); bRSurface = IMG_Load("res/BR.png"); bNSurface = IMG_Load("res/BN.png");
    bBSurface = IMG_Load("res/BB.png"); bQSurface = IMG_Load("res/BQ.png"); bKSurface = IMG_Load("res/BK.png");
    wPSurface = IMG_Load("res/WP.png"); wRSurface = IMG_Load("res/WR.png"); wNSurface = IMG_Load("res/WN.png");
    wBSurface = IMG_Load("res/WB.png"); wQSurface = IMG_Load("res/WQ.png"); wKSurface = IMG_Load("res/WK.png");
    ...
    dstTitleRect = {boardTextureRectOriginx + 1040, boardTextureRectOriginy, titleHeight, titleWidth};
    srcBoardRect = {0, 0, boardWidth, boardHeight}; 
    dstBoardRect = {boardTextureRectOriginx, boardTextureRectOriginy, 1000, 1000};
    srcPieceRect = {0, 0, boardSpaceTextureRectWidth, boardSpaceTextureRectHeight};
    srcPromotionPieceRect = {0, 0, boardSpaceTextureRectWidth / 2, boardSpaceTextureRectHeight / 2};
    srcWhiteSquare = {80, 80, 100, 100};

    /* RECTANGLES FOR EACH SPACE ON THE BOARD */
    boardSpaceRects.push_back(&A8Rect); boardSpaceRects.push_back(&B8Rect); boardSpaceRects.push_back(&C8Rect); boardSpaceRects.push_back(&D8Rect); 
    boardSpaceRects.push_back(&E8Rect); boardSpaceRects.push_back(&F8Rect); boardSpaceRects.push_back(&G8Rect); boardSpaceRects.push_back(&H8Rect);
    boardSpaceRects.push_back(&A7Rect); boardSpaceRects.push_back(&B7Rect); boardSpaceRects.push_back(&C7Rect); boardSpaceRects.push_back(&D7Rect); 
    boardSpaceRects.push_back(&E7Rect); boardSpaceRects.push_back(&F7Rect); boardSpaceRects.push_back(&G7Rect); boardSpaceRects.push_back(&H7Rect);
    boardSpaceRects.push_back(&A6Rect); boardSpaceRects.push_back(&B6Rect); boardSpaceRects.push_back(&C6Rect); boardSpaceRects.push_back(&D6Rect); 
    boardSpaceRects.push_back(&E6Rect); boardSpaceRects.push_back(&F6Rect); boardSpaceRects.push_back(&G6Rect); boardSpaceRects.push_back(&H6Rect);
    boardSpaceRects.push_back(&A5Rect); boardSpaceRects.push_back(&B5Rect); boardSpaceRects.push_back(&C5Rect); boardSpaceRects.push_back(&D5Rect); 
    boardSpaceRects.push_back(&E5Rect); boardSpaceRects.push_back(&F5Rect); boardSpaceRects.push_back(&G5Rect); boardSpaceRects.push_back(&H5Rect);
    boardSpaceRects.push_back(&A4Rect); boardSpaceRects.push_back(&B4Rect); boardSpaceRects.push_back(&C4Rect); boardSpaceRects.push_back(&D4Rect); 
    boardSpaceRects.push_back(&E4Rect); boardSpaceRects.push_back(&F4Rect); boardSpaceRects.push_back(&G4Rect); boardSpaceRects.push_back(&H4Rect);
    boardSpaceRects.push_back(&A3Rect); boardSpaceRects.push_back(&B3Rect); boardSpaceRects.push_back(&C3Rect); boardSpaceRects.push_back(&D3Rect); 
    boardSpaceRects.push_back(&E3Rect); boardSpaceRects.push_back(&F3Rect); boardSpaceRects.push_back(&G3Rect); boardSpaceRects.push_back(&H3Rect);
    boardSpaceRects.push_back(&A2Rect); boardSpaceRects.push_back(&B2Rect); boardSpaceRects.push_back(&C2Rect); boardSpaceRects.push_back(&D2Rect); 
    boardSpaceRects.push_back(&E2Rect); boardSpaceRects.push_back(&F2Rect); boardSpaceRects.push_back(&G2Rect); boardSpaceRects.push_back(&H2Rect);
    boardSpaceRects.push_back(&A1Rect); boardSpaceRects.push_back(&B1Rect); boardSpaceRects.push_back(&C1Rect); boardSpaceRects.push_back(&D1Rect); 
    boardSpaceRects.push_back(&E1Rect); boardSpaceRects.push_back(&F1Rect); boardSpaceRects.push_back(&G1Rect); boardSpaceRects.push_back(&H1Rect);
    ...
    for (auto const& boardSpace : boardSpaceRects)
    {
        *boardSpace = {boardSpaceTextureRectOriginx + iteratorx * boardSpaceWidth,
            boardSpaceTextureRectOriginy + iteratory * boardSpaceHeight, boardSpaceTextureRectWidth,
            boardSpaceTextureRectHeight};
...
void Game_c::initializeGameDisplay(const char* title, int displayLocX, int displayLocY, 
    ...
    titleTexture = SDL_CreateTextureFromSurface(renderer, titleSurface); SDL_FreeSurface(titleSurface);
    boardTexture = SDL_CreateTextureFromSurface(renderer, boardSurface); SDL_FreeSurface(boardSurface);
    wPTex = SDL_CreateTextureFromSurface(renderer, wPSurface); SDL_FreeSurface(wPSurface);
    bPTex = SDL_CreateTextureFromSurface(renderer, bPSurface); SDL_FreeSurface(bPSurface);
    wRTex = SDL_CreateTextureFromSurface(renderer, wRSurface); SDL_FreeSurface(wRSurface);
    bRTex = SDL_CreateTextureFromSurface(renderer, bRSurface); SDL_FreeSurface(bRSurface);
    wNTex = SDL_CreateTextureFromSurface(renderer, wNSurface); SDL_FreeSurface(wNSurface);
    bNTex = SDL_CreateTextureFromSurface(renderer, bNSurface); SDL_FreeSurface(bNSurface);
    wBTex = SDL_CreateTextureFromSurface(renderer, wBSurface); SDL_FreeSurface(wBSurface);
    bBTex = SDL_CreateTextureFromSurface(renderer, bBSurface); SDL_FreeSurface(bBSurface);
    wQTex = SDL_CreateTextureFromSurface(renderer, wQSurface); SDL_FreeSurface(wQSurface);
    bQTex = SDL_CreateTextureFromSurface(renderer, bQSurface); SDL_FreeSurface(bQSurface);
    wKTex = SDL_CreateTextureFromSurface(renderer, wKSurface); SDL_FreeSurface(wKSurface);
    bKTex = SDL_CreateTextureFromSurface(renderer, bKSurface); SDL_FreeSurface(bKSurface);
    whiteSquareTex = SDL_CreateTextureFromSurface(renderer, whiteSquareSurface); SDL_FreeSurface(whiteSquareSurface);

    /* CHECKMATE */
    checkmateTex = SDL_CreateTextureFromSurface(renderer, checkmateSurface); SDL_FreeSurface(checkmateSurface);

    /* UNDO FUNCTIONALITY */
    undoRectTex = SDL_CreateTextureFromSurface(renderer, undoRectSurface); SDL_FreeSurface(undoRectSurface);

    /* DEBUGGING */
    /* debug button */
    debugRectTex = SDL_CreateTextureFromSurface(renderer, debugRectSurface); SDL_FreeSurface(debugRectSurface);
    /* print board button */
    printBoardRectTex = SDL_CreateTextureFromSurface(renderer, printBoardSurface); SDL_FreeSurface(printBoardSurface);
    /* print piece info button */
    printPieceInfoRectTex = SDL_CreateTextureFromSurface(renderer, printPieceInfoSurface); SDL_FreeSurface(printPieceInfoSurface);
    /* toggle AI button */
    toggleAIRectTex = SDL_CreateTextureFromSurface(renderer, toggleAISurface); SDL_FreeSurface(toggleAISurface);

    /* DIFFICULTY SELECT */
    difficultyTex = SDL_CreateTextureFromSurface(renderer, difficultySurface); SDL_FreeSurface(difficultySurface);
    veryEasyTex = SDL_CreateTextureFromSurface(renderer, veryEasySurface); SDL_FreeSurface(veryEasySurface);
    easyTex = SDL_CreateTextureFromSurface(renderer, easySurface); SDL_FreeSurface(easySurface);
    mediumTex = SDL_CreateTextureFromSurface(renderer, mediumSurface); SDL_FreeSurface(mediumSurface);
    hardTex = SDL_CreateTextureFromSurface(renderer, hardSurface); SDL_FreeSurface(hardSurface);
}
        ...
        case SDL_MOUSEBUTTONDOWN:
        {
            if (!isPromotionPossible)
            {
                ...
                if (SDL_PointInRect(&mousePosition, &dstDebugRect))
                    int debug = 0;                                              // this is used as a breakpoint so you can start debugging and adding breakpoints at will
...
void Game_c::gameLoop(){
    ...
    while(gameState_ec == GameState_ec::PLAY)
    {
        if (isInCheckmate)
        {
            SDL_RenderCopy(renderer, checkmateTex, &srcCheckmateBox, &dstCheckmateBox);
        ...
        {
            SDL_RenderCopy(renderer, whiteSquareTex, &srcWhiteSquare, boardSpaceRects[move.to]);
        }

        for (auto const& specialMove : possibleSpecialMoves[setDepth])
        {
            SDL_RenderCopy(renderer, whiteSquareTex, &srcWhiteSquare, boardSpaceRects[specialMove.to1]);
        }
        ...
        for (auto const& piece : pieceInfo)
        {
            ...
            {
                switch (piece.figure & 95 /* pull the type, color, and passant info of each piece */)
            {
                case PAWN:
                    SDL_RenderCopy(renderer, wPTex, &srcPieceRect, boardSpaceRects[piece.space]);
                    break;

                case PASSANT| PAWN:
                    // SDL_RenderCopy(renderer, whiteSquareTex, &srcPieceRect, boardSpaceRects[piece.space]);
                    SDL_RenderCopy(renderer, wPTex, &srcPieceRect, boardSpaceRects[piece.space]);
                    break;

                case BLACK | PAWN:
                    SDL_RenderCopy(renderer, bPTex, &srcPieceRect, boardSpaceRects[piece.space]);
                    break;
                ...
                case ROOK:
                    SDL_RenderCopy(renderer, wRTex, &srcPieceRect, boardSpaceRects[piece.space]);
                    break;

                case BLACK | ROOK:
                    SDL_RenderCopy(renderer, bRTex, &srcPieceRect, boardSpaceRects[piece.space]);
                    break;

                case KNIGHT:
                    SDL_RenderCopy(renderer, wNTex, &srcPieceRect, boardSpaceRects[piece.space]);
                    break;

                case BLACK | KNIGHT:
                    SDL_RenderCopy(renderer, bNTex, &srcPieceRect, boardSpaceRects[piece.space]);
                    break;
                
                case BISHOP:
                    SDL_RenderCopy(renderer, wBTex, &srcPieceRect, boardSpaceRects[piece.space]);
                    break;

                case BLACK | BISHOP:
                    SDL_RenderCopy(renderer, bBTex, &srcPieceRect, boardSpaceRects[piece.space]);
                    break;

                case QUEEN:
                    SDL_RenderCopy(renderer, wQTex, &srcPieceRect, boardSpaceRects[piece.space]);
                    break;

                case BLACK | QUEEN:
                    SDL_RenderCopy(renderer, bQTex, &srcPieceRect, boardSpaceRects[piece.space]);
                    break;

                case KING:
                    SDL_RenderCopy(renderer, wKTex, &srcPieceRect, boardSpaceRects[piece.space]);
                    break;

                case BLACK | KING:
                    SDL_RenderCopy(renderer, bKTex, &srcPieceRect, boardSpaceRects[piece.space]);
                    break;
...
int Game_c::minimax(int depth, int minimaxTurn)
{
    ...
    if (minimaxTurn == maximizingPlayer)
    {
        ...
        {
            makeMove(move);
            // if (pieceInfo[move.piece].figure & FIGURE == PAWN && (move.to/8 == 7 || move.to/8 == 0))
            // {
...
void Game_c::getPawnMoves(int piece, int from)
{
    ...
    for (int i = -1; i < 2; i+=2)
    {
        ...
        bool isPassantAvailable = ((pieceInfo[possiblePassantPiece].figure & PASSANT) == PASSANT)
                                    ? true : false;
        if (isPassantAvailable)
        {
            int moveOffset = (color == BLACK) ? 8 : -8;
            Special_Move_s special_move = createSpecialMove(piece, possiblePassantPiece,
...
void Game_c::getKingMoves(int piece, int from)
{
    ...
    if (castlePossible[0])
    {
        Special_Move_s specialMove = createSpecialMove(piece, leftRookIdentifier, from - 3, from - 2, from, from - 4);
        addSpecialMoveIfValid(specialMove);
    }
    if (castlePossible[1])
    {
        Special_Move_s specialMove = createSpecialMove(piece, rightRookIdentifier, from + 2, from + 1, from, from + 3);
        addSpecialMoveIfValid(specialMove);
    }
}
...
Special_Move_s Game_c::createSpecialMove(int piece1, int piece2, int to1, int to2, int from1, int from2)
{
...
void Game_c::makeMove(Move_s move)
{
    ...
    if ((pieceInfo[move.piece].figure & FIGURE) == PAWN && abs(move.from - move.to) == 16) newFigure = SET_PASSANT(newFigure);
...
void Game_c::checkForCheckmate()
{
    ...
    if (possibleMoves[setDepth].size() == 0 && possibleSpecialMoves[setDepth].size() == 0)
        isInCheckmate = true;
    clearPossibleMoves();
    clearPossibleSpecialMoves();
    }
...
void Game_c::updatePromotionStatus(int piece)
{
    ...
    *promotionRectVector[2] = {rectStartingX, rectStartingY + boardSpaceTextureRectHeight / 2, 
                                 boardSpaceTextureRectWidth / 2, boardSpaceTextureRectHeight / 2};

    *promotionRectVector[3] = {rectStartingX + boardSpaceTextureRectWidth / 2, rectStartingY + boardSpaceTextureRectHeight / 2, 
                                 boardSpaceTextureRectWidth / 2, boardSpaceTextureRectHeight / 2};                            
}
...
void Game_c::dropPiece()
{
    ...
    for (auto const& rect : boardSpaceRects)
    {
        if (SDL_PointInRect(&mousePosition, rect) && iterator != 64 && grabbedPieceLog.identifier != EMPTY)
        {
            ...
            for (auto const& move : possibleMoves[currentDepth])
            {
                ...
                if (isValidMove)
                {
                    ...
                    clearGrabbedPiece();
                    turn ^= BLACK;
                    if ((pieceInfo[moveLog.piece1].figure & FIGURE) == PAWN && (moveLog.to1 / 8 == 0 || moveLog.to1 / 8 == 7))
                    {
                        updatePromotionStatus(moveLog.piece1);
                    }
                    std::cout << "Moved piece figure: " << pieceInfo[moveLog.piece1].figure << std::endl;
                    logUndo();
                    clearPossibleMoves();
                    clearPossibleSpecialMoves();
                    checkForCheckmate();
...
void Game_c::toggleAI()
{
    if (aiPlayer == BLACK || aiPlayer == WHITE)
        aiPlayer = 1;                               // this will turn the AI off since it is neither black nor white
    else
    {
        aiPlayer = BLACK;                           // the ai can only be set back to black
    }
}
